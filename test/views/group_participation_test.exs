defmodule Bonfire.UI.Groups.GroupParticipationTest do
  ...
  test "separate membership and feed controls preserve independent states" do
    ...
    {:ok, group} = Categories.create(creator, %{name: Faker.Lorem.sentence(), username: "participation_#{System.unique_integer([:positive])}", type: :group, membership: "open", visibility: "global"}, true)
    {:ok, _} = Categories.join_group(user, group)
    ...
    session =
      ...
      |> assert_has("div.tooltip[data-tip='Unfollow group'] button#follow_feed_#{group.id}.bg-transparent.text-primary[aria-label='Unfollow group'][aria-pressed='true']")
      |> assert_has("#follow_feed_#{group.id} [iconify='ph:rss']")
      |> click_button("button#follow_feed_#{group.id}[data-id=unfollow]", "")
      |> assert_has("#join_btn_#{group.id}", text: "Joined")
      |> assert_has("div.tooltip[data-tip='Follow group'] button#follow_feed_#{group.id}.bg-transparent.text-muted[aria-label='Follow group'][aria-pressed='false']")
  ...
  test "join requests can be cancelled from the group button" do
    ...
    user = fake_user!(account)
    creator = fake_user!(account)
    {:ok, group} = Categories.create(creator, %{name: Faker.Lorem.sentence(), username: "request_#{System.unique_integer([:positive])}", type: :group, membership: "on_request", visibility: "global"}, true)
